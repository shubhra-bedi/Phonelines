            ...
            for call in data:
                for customer in customers:
                    if customer.get_id() == int(filter_string):
                        outgoing_call = customer.get_history()[1]
                        incoming_call = customer.get_history()[0]
                        if call in outgoing_call:
                            lst.append(call)
                        if call in incoming_call:
                            lst.append(call)
            return lst
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        try:
            for call in data:
                if filter_string[:1] == 'L' or filter_string[:1] == 'l':
                    call_time = int(filter_string[1:])

                    for customer in customers:
                        if call in customer.get_history()[0] or\
                                customer.get_history()[1]:
                            if call.duration < call_time:
                                lst.append(call)
                    print(lst)
                    return lst

                elif filter_string[:1] == 'G' or filter_string[:1] == 'g':
                    call_time = int(filter_string[1:])
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        specified in the handout.
        """
        # basically filter string is lowerlong, lowerlat, upperlong, upperlat
        ...
        for call in data:
            src_long = call.src_loc[0]
            src_lat = call.src_loc[1]
            if lower_long < src_long < upper_long and \
                    lower_lat < src_lat < upper_lat:
                lst.append(call)

            dst_long = call.dst_loc[0]
            dst_lat = call.dst_loc[1]
            if lower_long < dst_long < upper_long and \
